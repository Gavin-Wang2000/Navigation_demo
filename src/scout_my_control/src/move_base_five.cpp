...
void shutdown(int sig)
{
  ...
  ROS_INFO("nav_pentagon.cpp ended!");
  ros::shutdown();
}
...
{
  ros::init(argc, argv, "nav_move_base_pentagon");
  ...
  ROS_INFO("move_base_pentagon.cpp start...");

  // How big is the pentagon we want the robot to navigate?
  double pentagon_size = 1.0;
  ...
  for (int angle_count = 0; angle_count < 5; angle_count++)
  {
  ...
  point.x = pentagon_size * cos(0);
  point.y = pentagon_size * sin(0);
  point.z = 0.0;
  pose_list[0].position = point;
  pose_list[0].orientation = quaternions[0];

  point.x = pentagon_size * cos(2 * M_PI / 5);
  point.y = pentagon_size * sin(2 * M_PI / 5);
  point.z = 0.0;
  pose_list[1].position = point;
  pose_list[1].orientation = quaternions[1];

  point.x = pentagon_size * cos(4 * M_PI / 5);
  point.y = pentagon_size * sin(4 * M_PI / 5);
  point.z = 0.0;
  pose_list[2].position = point;
  pose_list[2].orientation = quaternions[2];

  point.x = pentagon_size * cos(6 * M_PI / 5);
  point.y = pentagon_size * sin(6 * M_PI / 5);
  point.z = 0.0;
  pose_list[3].position = point;
  pose_list[3].orientation = quaternions[3];

  point.x = pentagon_size * cos(8 * M_PI / 5);
  point.y = pentagon_size * sin(8 * M_PI / 5);
  point.z = 0.0;
  ...
  int count = 0;
  // Cycle through the five waypoints
